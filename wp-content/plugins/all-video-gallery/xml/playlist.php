...
function buildNodes() {
	...
	$items = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "allvideogallery_videos WHERE published=1 AND category='" . $video->category . "' AND id!=" . $_GET['vid']);
	$node  = '';
	...
	for ($i = 0, $n = count($items); $i < $n; $i++) {
		...
		$node .= '<link>'.@add_query_arg( "slg", $item->slug, $page ).'</link>'."\n";
		$node .= '</item>'."\n";
	}
