...
function allvideogallery_db_install() {
	...
	if(!get_site_option('allvideogallery_version')) {
		...
		$sql = "CREATE TABLE " . $table_name . " (
			...
			`custom_page_url` varchar(255) NOT NULL,
			UNIQUE KEY (`id`)
		);";
   		dbDelta($sql);
...
function allvideogallery_db_install_data() {
	...
	if(!get_site_option('allvideogallery_version')) {
		...
		add_option( "allvideogallery_version", "1.1" );
	}
}
...
/* Check for Update
******************************************************************/
function allvideogallery_update_db_check() {
	global $wpdb;
	
    if (get_site_option('allvideogallery_version') != "1.1") {
	 	$table_name = $wpdb->prefix . "allvideogallery_profiles";
		$sql = $wpdb->prepare( "ALTER TABLE " . $table_name . " ADD `custom_page_url` VARCHAR(255) NOT NULL AFTER `gallery_css`" );
   		$wpdb->query( $sql );
        update_option( "allvideogallery_version", "1.1" );
    }
}
