...
/* ===== template and filter functions ===== */
	...
	*/
	function shortcode($atts) {
		...
		$subscribe_button_value = apply_filters('s2_subscribe_button', __('Submit', 'subscribe2'));
		...
		if ( $hide == 'subscribe' ) {
			...
			$this->input_form_action = "<input class=\"btn-subscribe\"  type=\"submit\" name=\"subscribe\" value=\"" . esc_attr($subscribe_button_value) . "\" />";
		} else {
			// both form input actions
			$this->input_form_action = "<input class=\"btn-subscribe\" type=\"submit\" name=\"subscribe\" value=\"" . esc_attr($subscribe_button_value) . "\" />&nbsp;<input type=\"submit\" name=\"unsubscribe\" value=\"" . esc_attr($unsubscribe_button_value) . "\" />";
		}
		...
		// build default form
		if ( $nojs == 'true' ) {
			$this->form = "<form class=\"newsletter-form\" method=\"post\" action=\"" . $url . "\"><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" /><input type=\"text\" name=\"email\" id=\"s2email\" value=\"\" size=\"" . $size . "\" />" . $this->input_form_action . "</form>";
		} else {
			$this->form = "<form class=\"newsletter-form\" method=\"post\" action=\"" . $url . "\"><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" /><input type=\"text\" name=\"email\" id=\"s2email\" value=\"" . __('', 'subscribe2') . "\" size=\"" . $size . "\" onfocus=\"if (this.value == '" . __('Enter email address...', 'subscribe2') . "') {this.value = '';}\" onblur=\"if (this.value == '') {this.value = '" . __('', 'subscribe2') . "';}\" />" . $this->input_form_action . "</form>\r\n";
		}
